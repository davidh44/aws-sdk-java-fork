        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("Created", targetDepth)) {
                    context.nextToken();
                    connection.setCreated(DateJsonUnmarshallerFactory.getInstance("iso8601").unmarshall(context));
                }
                if (context.testExpression("Id", targetDepth)) {
                    context.nextToken();
                    connection.setId(context.getUnmarshaller(String.class).unmarshall(context));
                }
